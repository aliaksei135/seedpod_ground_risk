...
class PlotServer:
    ...
    def __init__(self, tiles: str = 'Wikipedia', tools: Optional[Iterable[str]] = None,
        ...
        self.plot_on_action = True
        self._plot_action_allowed = True
        self.callback_streams = [RangeXY(subscribers=[self._update_bounds])]
    ...
    def _update_bounds(self, x_range, y_range):
        # Add a constant unnoticeable skew to the plot ranges
        # Bokeh memoises existing frames generated by the stream kwargs they were generated with.
        # As a new frame was generated when the user panned/zoomed the map in order to render the map tiles,
        # this frame was memoised as just the map tiles. If you try to call .event(...) with the same map ranges,
        # Bokeh will not hit the callable to generate the risk layers, instead returning the cached map tiles.
        # We work around this by modifying map ranges enough for Bokeh to notice but not enough for the user perceive
        const_skew = 1e-9
        self._x_range = (x_range[0] - const_skew, x_range[1] + const_skew)
        self._y_range = (y_range[0] - const_skew, y_range[1] + const_skew)
